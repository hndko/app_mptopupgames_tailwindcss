<template>
  <div class="space-y-10 sm:space-y-12 pb-16">
    <!-- Header Hero Section -->
    <section class="relative overflow-hidden rounded-3xl bg-gradient-to-b from-slate-900 via-slate-900/90 to-slate-950 border border-slate-800 p-6 sm:p-10 text-center shadow-2xl">
      <div class="max-w-3xl mx-auto space-y-3">
        <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-sky-500/10 border border-sky-500/30 text-sky-400 text-xs font-black tracking-wider uppercase">
          ...
          <span>Gaming Tools &amp; Estimator</span>
        </div>
        <h1 class="text-2xl sm:text-4xl font-black text-white tracking-tight">
          Kalkulator <span class="text-sky-400">Mobile Legends</span>
        </h1>
        <p class="text-xs sm:text-sm text-slate-300 leading-relaxed">
          Hitung target Win Rate push rank, estimasi kebutuhan diamond Magic Wheel skin Legend, dan draw skin Zodiac secara akurat.
        </p>

        <!-- Calculator Tab Switcher -->
        <div class="inline-flex p-1.5 rounded-2xl bg-slate-950/80 border border-slate-800 gap-1.5 shadow-inner mt-4 overflow-x-auto max-w-full">
          <button 
            type="button"
            v-for="tab in calcTabs" 
            :key="tab.id"
            @click="activeTab = tab.id"
            :class="[activeTab === tab.id ? 'bg-sky-600 text-white shadow-md shadow-sky-600/30' : 'text-slate-400 hover:text-white hover:bg-slate-900']"
            class="px-4 sm:px-6 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap btn-press"
          >
            <i :class="tab.icon"></i>
            <span>{{ tab.label }}</span>
          </button>
    ...
    <!-- ==================================================================== -->
    <!-- 1. TAB: KALKULATOR WIN RATE MLBB -->
    <!-- ==================================================================== -->
    <div v-if="activeTab === 'winrate'" class="max-w-4xl mx-auto space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        <!-- Input Form (7 cols) -->
        <div class="md:col-span-7 bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5">
          <div class="flex items-center gap-3 pb-3 border-b border-slate-800">
            ...
            <div>
              <h3 class="text-base font-black text-white">Hitung Kemenangan Beruntun</h3>
              <p class="text-xs text-slate-400">Masukkan statistik profil akun MLBB Anda.</p>
            </div>
          </div>
        ...
        <!-- Result Display Card (5 cols) -->
        <div class="md:col-span-5 bg-gradient-to-b from-sky-950/40 via-slate-900 to-slate-900 border border-sky-800/60 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-2xl space-y-6">
          <div class="space-y-4">
            ...
            <div v-if="wrResult.status === 'valid'" class="space-y-3">
              ...
              <div class="text-[11px] text-slate-400 space-y-1 pt-2">
                ...
                <div class="flex justify-between">
                  ...
                  <span class="text-white font-bold font-mono">{{ wrResult.finalWins }} Wins</span>
                </div>
              </div>
            </div>
            ...
            <div v-else class="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 text-slate-400 text-xs text-center">
              Isi parameter di samping untuk melihat hasil kalkulasi.
            </div>
          </div>

          <div class="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800 text-[11px] text-slate-400 flex items-center gap-2">
            <i class="fas fa-lightbulb text-amber-400 shrink-0"></i>
            <span>Tips: Mabar bersama squad terpercaya untuk menjaga konsistensi win streak ranked.</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- ==================================================================== -->
    <!-- 2. TAB: KALKULATOR MAGIC WHEEL MLBB -->
    <!-- ==================================================================== -->
    <div v-if="activeTab === 'magic-wheel'" class="max-w-4xl mx-auto space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        <!-- Input Form (7 cols) -->
        <div class="md:col-span-7 bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5">
          <div class="flex items-center gap-3 pb-3 border-b border-slate-800">
            ...
            <div>
              <h3 class="text-base font-black text-white">Hitung Kebutuhan Magic Wheel</h3>
          ...
          <div class="space-y-4">
            <div>
              <div class="flex justify-between items-center mb-1">
                <label for="magicPointInput" class="block text-xs font-semibold text-slate-300">
                  Magic Point Saat Ini (0 - 200)
                </label>
                <span class="text-xs font-black text-amber-400 font-mono">{{ mwPoint }} / 200</span>
              </div>
              <input 
                type="range" 
                id="magicPointRange"
        ...
        <!-- Result Display Card (5 cols) -->
        <div class="md:col-span-5 bg-gradient-to-b from-amber-950/30 via-slate-900 to-slate-900 border border-amber-500/40 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-2xl space-y-6">
          <div class="space-y-4">
    ...
    <!-- ==================================================================== -->
    <!-- 3. TAB: KALKULATOR ZODIAC MLBB -->
    <!-- ==================================================================== -->
    <div v-if="activeTab === 'zodiac'" class="max-w-4xl mx-auto space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        <!-- Input Form (7 cols) -->
        <div class="md:col-span-7 bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5">
          <div class="flex items-center gap-3 pb-3 border-b border-slate-800">
          ...
          <div class="space-y-4">
            <div>
              <div class="flex justify-between items-center mb-1">
                <label for="starPowerInput" class="block text-xs font-semibold text-slate-300">
        ...
        <!-- Result Display Card (5 cols) -->
        <div class="md:col-span-5 bg-gradient-to-b from-purple-950/30 via-slate-900 to-slate-900 border border-purple-500/40 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-2xl space-y-6">
          <div class="space-y-4">
            ...
            <div class="space-y-3">
              <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-2">
                ...
                <div class="flex justify-between text-xs">
                  <span class="text-slate-400">Estimasi Rata-rata Draw 5x:</span>
